 ...
 */
void resetReceiveState(uint8_t sourceIDState){
  extern uint8_t* RXBufferCurrentPosition; 
  extern uint8_t  RXStateFlags;
  extern uint8_t RXBufferContentSourceID;
  ...
  if(sourceIDState & COM_SET_SCI0_INTERFACE_ID){
    ...
    /* Only SCI for now */
    uint8_t devnull; // Is there a better way to do this?
...
void SCI0ISR(){
  ...
  if(flags & (SCISR1_RX_REGISTER_FULL | SCISR1_RX_OVERRUN)){
    ...
    if(SCI0CR2 & SCICR2_RX_ISR_ENABLE){
      if(flags & (SCISR1_RX_NOISE | SCISR1_RX_FRAMING | SCISR1_RX_PARITY | SCISR1_RX_OVERRUN)){
        ...
        }else if((uint16_t)RXBufferCurrentPosition >= ((uint16_t)&RXBuffer + RX_BUFFER_SIZE)){
  ...
  if((SCI0CR2 & SCICR2_TX_ISR_ENABLE) && (flags & SCISR1_TX_REGISTER_EMPTY)){
    ...
    uint8_t rawValue = *TXBufferCurrentPositionSCI0;
