...
//static uint8_t waitCycles = 44;
static uint16_t testCH = 0;
static uint32_t delay = 2500;
static uint16_t runtime = 5000; //0x0FFF; //0x0FFE - 40;
static uint32_t delay2 = 10000;
static uint32_t sleep = 10;
static uint16_t numRuns = 0;
...
//    *xgsInStamp = TC0;
//    *xgsEventsToSch = 6;
//    //*(uint16_t*)(eventFlags1 - 0x8000) = (uint16_t)0;
//    //*(uint16_t*)(eventFlags1 - 0x8000) = (uint16_t)0;
//    delay++;
//    //runtime++;
//    XGOutputEvents[0].channelID = testCH;
...
//    if(numRuns % 10 == 0) {
//ATOMIC_START();
//    uint32_t potentialDelay = 0x0FFF;
//  //  outputEventInputEventNumbers[numRuns] = mappedEvent;
//    outputEventDelayFinalPeriod[testCH] = (uint16_t)potentialDelay;
...
#define TIME_DIVISOR  50UL //6000 rpm
  if (Clocks.millisToTenths % TIME_DIVISOR == 0) {
    ATOMIC_START();
    *ticksPerDegree = (uint16_t)((ticks_per_degree_multiplier * (1250UL * TIME_DIVISOR) ) / (720UL * ANGLE_FACTOR));
