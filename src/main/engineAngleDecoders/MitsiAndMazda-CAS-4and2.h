...
#define TOTAL_INPUTS                (TOTAL_CRANK_TEETH * 2)

#define DEGREES_PER_CRANK_ROTATION  (360)  //TODO move to lib section
//TODO replace ABT with inline function
#define ANGLE_BETWEEN_TEETH         ENGINE_ANGLE_S((360 / TOTAL_CRANK_TEETH))

extern uint16_t crankAngles[TOTAL_INPUTS];

static inline uint16_t getAngle(uint8_t index) {
  return crankAngles[index];
}
