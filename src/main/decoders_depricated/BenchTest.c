...
void PrimaryEngineAngle(){
	...
	uint8_t shouldFire = 1;
//	uint32_t localPeriod = 0; // mutlifire or busy wait, if doing this, check last period for some min, and if too small, shrink second to last and increase last
//	uint16_t localPeriod = 0; // normal mode
	if(testMode == TEST_MODE_ITERATIONS){
		...
		if(fakeCurrentEvent < testNumberOfMissing){
			...
			uint16_t singleWidth = testTicksPerEvent/2;
			...
			if((KeyUserDebugs.currentEvent == ((testEventsPerCycle / 2) - 1)) || (KeyUserDebugs.currentEvent == (testEventsPerCycle - 1))){
				...
				uint32_t wideWideWidth = (uint32_t)singleWidth * (testNumberOfMissing + 1);
				if(wideWideWidth < SHORTMAX){
					outputEventPulseWidthsMath[0] = (uint16_t)wideWideWidth;
				}else{
