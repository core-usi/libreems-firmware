...
#define BLOCK_IS_2DUL_TABLE        BIT10_16
 ...
 */
typedef struct {
  uint16_t flags;    ///< Property flags, see flag masks above
  uint16_t parent;   ///< Location ID of parent block if sub block
  uint8_t RAMPage;   ///< RAM page number
  uint8_t FlashPage; ///< Flash page number
  void* RAMAddress;        ///< RAM address
  void* FlashAddress;      ///< Flash address
  uint16_t size;     ///< Memory block size
  uint8_t descriptorID;    ///< 8-bit identifier
} blockDetails;
