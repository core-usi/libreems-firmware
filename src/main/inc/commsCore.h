...
void sendDataStreamDescriptor(void) TEXT;
void sendTableDescriptor(void) TEXT;

void sendFieldDescriptor(void) TEXT;
void prepForTX(void) TEXT;

void resetReceiveState(uint8_t) FPAGE_FE;
void finaliseAndSend(uint16_t) FPAGE_FE;

uint16_t populateBasicDatalog(void) FPAGE_FE;
void prep_packet_with_length_ptr(uint16_t, uint16_t **) FPAGE_FE;
void prep_packet_with_no_length(uint16_t) FPAGE_FE;
...
extern const uint8_t interfaceVersion[INTERFACE_VERSION_LENGTH];
extern const uint8_t firmwareVersion[FIRMWARE_VERSION_LENGTH];
extern const uint8_t buildTimeAndDate[FIRMWARE_BUILD_DATE_LENGTH];
extern const uint8_t compilerVersion[COMPILER_VERSION_LENGTH];
extern const uint8_t operatingSystem[OPERATING_SYSTEM_LENGTH];
extern const uint16_t injectorSwitchOnCodeTime;
